...
export const parseID3tags = (songPath: string)=>{
    const fileBuffer = readFileSync(songPath)
    const songid3Tags: NodeID3.Tags = NodeID3.read(fileBuffer)
    console.info(songid3Tags)
}

export const updateID3tags = (songPath: string, tags: NodeID3.Tags) => {
    const success = NodeID3.update(tags, songPath);
    if (success instanceof Error) {
        throw new Error(`Erreur lors de la mise à jour des tags: ${success.message}`);
    }
    return success;
};
